...
def _generate_row(rng, is_fraudulent):
    ...
    if is_fraudulent:
        # Suspicious: price ABOVE the fair range (overpriced), missing details
        if rng.random() < 0.7:
            offset = rng.uniform(0.3, 1.5) * (predicted_price_max - predicted_price_min)
            # Only inflate above max — pricing below range is a good deal, not fraud
            price = predicted_price_max + offset
        else:
        ...
        # Genuine: price at or below fair range (good deal or fair price)
        price = rng.uniform(predicted_price_min * 0.85, predicted_price_max)
