...
class TrustScoreRequestSerializer(serializers.Serializer):
    ...
    completeness_score   = serializers.FloatField(min_value=0, max_value=100)
    # Optional — passed through to response for display, not used in scoring.
    seller_history       = SellerHistorySerializer(required=False)
