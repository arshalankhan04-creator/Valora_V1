...
// Mirrors listingIntelligence.js: fraudRiskScore = (1 - fraudProbability) * 100
function fraudRiskScoreFor(fraudProbability) {
  return Math.round((1 - fraudProbability) * 100)
}

// Mirrors trust_score/scoring.py's seller_bonus()
function sellerBonusFor({ responseRate, pastDeals, accountAgeDays }) {
  const response = (responseRate / 100) * 1.0
  const deals = (Math.min(pastDeals, 10) / 10) * 0.5
  const age = (Math.min(accountAgeDays, 365) / 365) * 1.0
  return round2(Math.min(5.0, response + deals + age))
}

// Mirrors trust_score/scoring.py's compute()
function computeTrust({ priceFairness, fraudRiskScore, conditionScore, sellerBonus }) {
  const priceFairnessWeighted = priceFairness * 0.4
  const fraudRiskWeighted = fraudRiskScore * 0.3
  const conditionWeighted = conditionScore * 0.3
  const trustScore = Math.min(100, round2(priceFairnessWeighted + fraudRiskWeighted + conditionWeighted + sellerBonus))
  return {
    trustScore,
    trustBreakdown: {
      priceFairness: round2(priceFairnessWeighted),
      fraudRisk: round2(fraudRiskWeighted),
      conditionMatch: round2(conditionWeighted),
      sellerFactor: sellerBonus,
    },
  }
}

function buildFraudReasons({ riskFlag, fairness, hasDescription, seller }) {
  ...
  if (seller.accountAgeDays < 30) reasons.push('Seller account is very new')
  if (seller.pastDeals === 0) reasons.push('Seller has no listing history')
  return reasons
}
...
async function seed() {
  ...
  for (let i = 0; i < LISTING_BLUEPRINTS.length; i++) {
    ...
    const fraudRiskScore = fraudRiskScoreFor(bp.fraud)
    const conditionScore = bp.condition ?? 100
    const sellerBonus = sellerBonusFor(sellerProfile)
    const { trustScore, trustBreakdown } = computeTrust({ priceFairness: fairness, fraudRiskScore, conditionScore, sellerBonus })
    ...
    const ml = {
      ...
      fraudReasons: buildFraudReasons({ riskFlag, fairness, hasDescription: !!bp.description, seller: sellerProfile }),
      trustScore,
      trustBreakdown,
    }
    if (bp.images > 0) {
